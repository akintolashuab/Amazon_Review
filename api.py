...
def single_prediction(predictor, scaler, cv, text_input):
    ...
    # === Text Cleaning Start ===
    review = text_input.lower()
    review = re.sub('\[.*?\]', '', review)  # remove content in square brackets
    review = re.sub('[%s]' % re.escape(string.punctuation), '', review)  # remove punctuations
    review = re.sub('\w*\d\w*', '', review)  # remove words containing digits
    review = re.sub('[ ‘’“”…]', '', review)  # remove specific characters (space, quotes, dots)
